...
class tfliteInference:
    ...
    """
    def __init__(self, model_path, pb_model_path=None,NpyPath='./', SavePath='./'):
...
def OnTake(thisTF):
    ...
    data = np.load(os.path.join(thisTF.NpyPath))

    weight = thisTF.get_weight(data)
    print(weight.shape)

    np.set_printoptions(threshold=np.inf)
    with open(thisTF.SavePath + 'weight.txt', 'w') as f:
...
if __name__ == "__main__":
    ...
    data = np.load(os.path.join('./lpc/1114_demo.npy'))
